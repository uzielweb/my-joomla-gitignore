...
- **Dumps de Banco de Dados (`/*.sql`)**: Dumps e backups de banco de dados colocados na raiz (`/*.sql`, `/*.sql.gz`, `/*.sql.zip`, etc.) são ignorados para proteger dados confidenciais e evitar inchar o repositório. No entanto, scripts SQL de instalação e migração dentro de extensões (como `/administrator/components/com_minha_extensao/sql/install.mysql.utf8.sql`) **são devidamente rastreados**.
...
- `/administrator/components/com_minha_extensao/sql/install.mysql.utf8.sql`
...
- **Database Dumps (`/*.sql`)**: Database dumps and backups placed at the root (`/*.sql`, `/*.sql.gz`, `/*.sql.zip`, etc.) are ignored to prevent leaking sensitive database data and repository bloat. However, SQL installation and migration scripts inside custom extensions (e.g., `/administrator/components/com_my_extension/sql/install.mysql.utf8.sql`) **are tracked**.
...
- `/administrator/components/com_my_extension/sql/install.mysql.utf8.sql`
